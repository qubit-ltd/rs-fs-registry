...
/// Verifies asynchronous registry diagnostics include catalog state.
#[test]
fn test_async_registry_implements_debug() {
    let debug = format!("{:?}", AsyncFileSystemRegistry::default());

    assert!(debug.contains("AsyncFileSystemRegistry"));
}
