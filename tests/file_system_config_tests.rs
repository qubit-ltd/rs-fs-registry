...
fn test_config_builder_preserves_validated_options_without_a_fallible_step() {
    ...
    let config = FileSystemConfig::new(FsUri::parse("mock:///file.txt").expect("URI should parse"))
...
fn test_config_debug_redacts_values_and_credential_references() {
    let config = FileSystemConfig::new(FsUri::parse("mock:///resource").expect("URI should parse"))
        .with_options(
            UserMetadata::new()
